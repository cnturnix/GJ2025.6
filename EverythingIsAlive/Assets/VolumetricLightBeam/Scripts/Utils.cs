...
namespace VLB
{
    public static class Utils
    {
        ...
        public static void GizmosDrawPlane(Vector3 normal, Vector3 position, Color color, Matrix4x4 mat, float size = 1f, float normalLength = 0.0f)
        {
            ...
            UnityEditor.Handles.RectangleHandleCap(0, position, Quaternion.LookRotation(normal), size, UnityEngine.EventType.Repaint);

            if (normalLength > 0.0f)
            {
                UnityEditor.Handles.DrawLine(position, position + normal * normalLength);
                UnityEditor.Handles.ConeHandleCap(0, position + normal * normalLength, Quaternion.LookRotation(normal), normalLength * 0.25f, UnityEngine.EventType.Repaint);
            }
